...
class sentinel2_msi_file(object):
    """
    #TODO generalise this class so it can be inherited for other sensors.
    """
    ...
    def get_correct_sizes(self, band_name, x, y):
        """
        Doesn't need to be used so much since resampling implementation. Looks up the correct size for a sentinel band (10,20 or 60 metre) and returns the true tile definitions for it.

        If band name is none then returns the 10 metre definitions
        """
    ...
    def read_tile(self, x, y):
        """
        Reads a tiles spectral data, returns a dictionary of spectral arrays.
        """
        bands = {}
    ...
    def define_tiles(self, width, height):
        """
        Creates tile boundaries, could be better implemented
        """
    ...
    def get_tile(self, tile_no):
        """
        Extracts a tile based on geopositioning.
        """
    ...
    def test_tile_water(self):
        """
        Uses basemap to check for water, should be updated to use the water mask generated by atmospheric correction
        """
        self.tile_water = False
        try:
    ...
    def generate_rgb_tile(self, output_location, r='B4', g='B3', b='B2'):
        """
        Takes the red green blue numpy arrays and stacks them into rgb image. This is much, much quicker than snappy, but visually the images are less useful.
        """
    ...
    def generate_snappy_rgb_tile(self, output_location):
        """
        Creates an rgb tile using snappys product reader, which also applies minor corrections resulting in a better visual image.
        """
        ...
        try:
            ...
            if chunk[1][0] + self.chunk_size >= self.height:
                self.true_chunk_def[1] = self.height - chunk[1][0]
            try:
    ...
    def make_spectral_nc(self, output_location):
        """
        Takes the current tile (set with self.tile) of spectral data and packages it into a compressed netcdf.

        Currently quite verbose.
        """
    ...
    def convert_rgb_to_jpg(self, png_file):
        """
        Does what it says, some models need three band images.
        """
    ...
    def do_resample(self):
        """
        Uses snappy to resample bands from 60 or 20 m to 10. Useful when we want to maintain high res.
    ...
    def do_idepix(self):
        """
        Does some basic masking, using idepix nn.
        """
        self.idepix_prod = GPF.createProduct('Idepix.Sentinel2',self.parameters,self.resamp)
    
    def export_mask_as_polygon(self, mask_name):
        """
        This is a shortcut to land/water/cloud classes, does mean we're training one nn with anothers output for cloud and some water. Others come from base masks.
    ...
    def make_text_definitions(self, output_location):
        """
        Chucks out some useful bounding box information.
        """
        chunk = self.chunks[self.current_tile]
    ...
    def make_tile_mask_json(self, output_location):
        """
        Chucks out the labels from idepix a json file for combination later with manual annotation.
        """
        labels = []
        object_id = 1
    ...
    def get_mask_realname(self, mask):
        """
        Placeholder for if there are ever masks we don't agree with the name for. Was used for cwater/cland in the past
        """
        if "cloud" in mask:
            return "cloud"
        if "water" in mask:
    ...
    def get_atm_corr_file(self, atm_corr_storage="/data/gfs03/scratch/stgo/atm_corrected_sen2", ancillary_dir="/data/datasets/operational/ancillary/obpg/live_mirror/Meteorological/"):
        """
        Runs polymers atmospheric correction on a sentinel granule and stores the output somewhere sensible. Should eventually be rebuilt to apply the atmospheric correction without resampling
        """
        print self.source_filename
    ...
    def create_rgb_for_all_tiles(self, output_location, r='B4', g='B3', b='B2', test_water=False):
        """
        Makes all the tiles rgb data and then outputs labels and spectral data.
        """
